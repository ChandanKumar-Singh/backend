...
class NotificationPreferenceDBO {
    ...
    async fetchNotificationPreferenceById(userId, { session = null } = {}) {
        const redisData = await Redis.hget(...RedisKeys.NOTIFICATION_PREFERENCE(userId));
    ...
    cacheUserPreference(userId, data) {
        Redis.hset(...RedisKeys.NOTIFICATION_PREFERENCE(userId), data);
    }
    ...
    purgeCache(userId) {
        Redis.hdel(...RedisKeys.NOTIFICATION_PREFERENCE(userId));
    }
}
