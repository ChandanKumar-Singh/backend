...
class PageController {
    list = catchAsync(async (req, res) => {
        ...
        res.status(200).json(resConv(data));
    });
    ...
    create = catchAsync(async (req, res) => withTransaction(async (session) => {
        ...
        res.status(200).json(resConv(page, { message: `${page.title} created successfully` }));
    }));
    ...
    updateWebPage = catchAsync(async (req, res) => withTransaction(async (session) => {
        ...
        res.status(200).json(resConv(page, { message: `${page.title} updated successfully` }));
    }));

    delete = catchAsync(async (req, res) => withTransaction(async (session) => {
        const { id } = req.body;
        const page = await WebPageDBO.delete(id, { session });
        res.status(200).json(resConv(page, { message: `${page.title} deleted successfully` }));
    }));
